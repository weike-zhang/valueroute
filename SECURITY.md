...
The v0.0.2 advisory routing surface is read-only: `/v1/advisory` only returns candidate suggestions and rejection reasons, never registers a controller, never creates a WorkerPlan, and never changes model or policy configuration. Shadow records store advice and a request fingerprint, not new write permissions or execution rights.
