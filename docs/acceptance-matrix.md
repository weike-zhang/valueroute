...
| FR-204 | pass | `src/valueroute/plugins/` exposes six stable structural contracts (Profiler, ControllerSelector, WorkerPolicy, Provider, Framework, Verifier) as runtime-checkable Protocols plus a `PluginRegistry` that validates role, name, contract version, and provider async shape before registration; third-party objects that do not satisfy the contract are rejected. |
| FR-205 | pass | `GET /v1/trace/ui` serves a read-only HTML runtime-trace page aggregating sessions, epochs (including automatic selection and switches), tasks, worker attempts, and shadow records; rendered values are HTML-escaped. |
| FR-301 | pass | `HandoffService.handoff_attempt` re-points a claimed WorkerAttempt to a target provider in read-only T1 mode (self-contained, low-risk), records the egress, and never grants the target provider controller authority; `POST /v1/tasks/{task_id}/handoff` is Idempotency-Key protected and replay-safe. |
| FR-302 | pass | `EgressPolicy` enforces field-level and data-classification-level egress rules (default public/internal only, restricted never leaves); `EgressLedger` journals every egress with source/target provider, fields, and classification, survives replay, and `GET /v1/egress` filters by task and target provider. |
